...
app.post('/FilterProduk', (req, res) => {
    var id_cat = req.body.filterCat
    var sql = `SELECT * FROM table_addproduk WHERE kategori="${id_cat}"`;
    dbs.query(sql, (err, result) => {
        if(err){
            throw err;
        } else {
    ...
    if(id_username !== '' && posting !== '' && harga !== '' && alamat !== '' && status !== '' && deskripsi !== '' && foto_produk !== '' && tanggaldibuat !== '' && foto_produk2 !== '' && foto_produk3 !== '')
    {
        ...
        filefoto_produk.mv("./tampungfile/" +foto_produk, (err) =>
        {
            ...
            else
            {
                ...
                var sql=`INSERT INTO table_addproduk VALUES ("${''}", "${id_username}", "${posting}", "${harga}", "${alamat}", "${status}", "${deskripsi}", "${foto_produk}", "${tanggaldibuat}", "${foto_produk2}", "${foto_produk3}")`;
                dbs.query(sql, (err, result)=>
                {
                    if(err)
                    {
...
app.get('/dataprofile/:id', (req, res) => {
    var id_user = req.params.id;
    var sql = `SELECT posting, harga, master_user_admin.namadepan, master_user_admin.foto_profile, table_addproduk.id, master_user_admin.alamat_user_admin, status, foto_produk, namadepan FROM table_addproduk INNER JOIN master_user_admin ON table_addproduk.id_username=master_user_admin.id WHERE id_username="${id_user}"`;
    dbs.query(sql, (err, result) => {
        if(err){
            throw err;
        } else {
...
app.get('/data_profile/:id', (req, res) => {
    var id_username = req.params.id;
    console.log(id_username)
    var sql = `SELECT * FROM master_user_admin WHERE id="${id_username}"`;
    dbs.query(sql, (err, result) => {
        if(err){
            throw err;
        } else {
